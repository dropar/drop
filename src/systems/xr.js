AFRAME.registerSystem('xr', {
  ...
  wrapSceneMethods: function () {
    document.getElementById('status').innerHTML += '<div> +++ xr wrapSceneMethods </div>';
    ...
    sceneEl.enterAR = function () {
      document.getElementById('status').innerHTML += '<div> +++ enterAR </div>';
      this.renderer.xr.startSession(self.lastARDisplay, 'ar', true);
    };
    sceneEl.exitAR = function () {
      document.getElementById('status').innerHTML += '<div> +++ exitAR </div>';
  ...
  cameraActivated: function () {
    document.getElementById('status').innerHTML += '<div> +++ xr cameraActivated </div>';
    var self = this;
  ...
  initXR: function (displays) {
    document.getElementById('status').innerHTML += '<div> +++ xr initXR </div>';
    var sceneEl = this.sceneEl;
  ...
  sessionStarted: function (data) {
    document.getElementById('status').innerHTML += '<div> +++ xr sessionStarted </div>';
  ...
  sessionEnded: function (data) {
    document.getElementById('status').innerHTML += '<div> +++ xr sessionEnded </div>';
    this.activeRealityType = 'magicWindow';
    this.el.emit('realityChanged', this.activeRealityType);
    if (this.activeRealityType === 'ar') {
      // To show camera on iOS devices
